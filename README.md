...
## 已結案搜尋＋開回（v20260804a）
佇列上方的搜尋框兩段式運作：輸入即本地過濾進行中案件（零網路）；本地零命中時（500ms debounce）才單發一筆伺服器查詢（`filterByFormula` 限 `進度狀態=已完成`，SEARCH 比對 姓名／LINE顯示名稱／Line 備註名稱，`maxRecords=10`，不預載不翻頁）。命中以**灰卡**呈現（「已結案・不在目前進行中清單內」，僅 🔗詳情＋開回 兩鍵，無等候徽章、不入佇列不入統計）。

**開回**＝confirm 後直寫 `PATCH {進度狀態: 人工接管中}` **單一欄位**（PAT 直連 Airtable）——刻意**不走** proxy 的 `restore`／`takeover` 動作：`恢復` 會推「感謝耐心等候🙏」給客戶；`接管` 在已完成紀錄上會穿透 no-op 守衛觸發 manual-takeover ack（下班時段真推客訊，`airtable_crm.py:2246` 一帶，屬 bot 端既存缺陷、另列 backlog）。開回不清 `結案日期`（保留歷史）、不動其他欄位；成功後刷新即進佇列可指派。取捨：無 proxy 稽核軌跡（backlog：proxy 靜默 `reopen` action）。

背景：2026-08-04 謝代書在戰情室找不到 陳彥甫／田禾麗——兩筆均為「已完成但案件實續跑」（近 7 天活躍的同類共 62 筆，多為 6/26 legacy 舊客群）。主查詢維持「進行中 或 近30天結案」不放寬＝防洪原則。
